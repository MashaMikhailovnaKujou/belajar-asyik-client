...
function renderHome() {
    ...
    const avatarAktif = (!window.currentUser || !window.currentUser.avatar || window.currentUser.avatar === "undefined") 
        ? "🐰" 
...
function showDetailMateri(topicId) {
    // 🔑 PERBAIKAN: Amankan ID topik/bab yang sedang aktif ke state global
    ...
    // Event Listener: Tombol Stop Jalannya Suara
    ...
    // Event Listener: Tombol Mulai Latihan Soal/Kuis Bab Ini
    document.getElementById("latihanTopikBtn")?.addEventListener("click", () => {
        if ('speechSynthesis' in window) window.speechSynthesis.cancel();
        startTopicQuiz(topicId);
    });
    
    // Event Listener: Tombol Kembali ke Daftar Sub-Materi
...
function renderProfil() {
    ...
    const avatarProfilAktif = (!window.currentUser || !window.currentUser.avatar || window.currentUser.avatar === "undefined") 
        ? "🐰" 
        : window.currentUser.avatar;

    // Ambil nama dan nomor absen dari user yang sedang login
    const namaSiswa = window.currentUser?.username || "Siswa Hebat";
    const nomorAbsen = window.currentUser?.no_absen || "-";
    ...
    contentDiv.innerHTML = `
        <div class="profile-section" style="flex-direction:column; text-align:center;">
            ...
            <h3 style="margin: 5px 0; color: #2c3e50;">👤 ${namaSiswa}</h3>
            <p style="margin: 5px 0; font-size: 1.1rem;">🔢 Nomor Absen: <strong>${nomorAbsen}</strong></p>
        ...
        <button class="logout-btn" id="logoutSiswaBtn" style="background: #EF5350; color: white; border: none; padding: 12px 20px; font-size: 1rem; font-weight: bold; border-radius: 40px; width: 100%; margin-top: 12px; cursor: pointer; box-shadow: 0 4px 10px rgba(239,83,80,0.3); transition: 0.2s;">
            🚪 Keluar Akun
        </button>
    `;
    ...
    // 🔑 EVENT LISTENER LOG OUT SISWA
    document.getElementById("logoutSiswaBtn")?.addEventListener("click", () => {
        if (confirm("Apakah kamu yakin ingin keluar dari aplikasi?")) {
            // Hapus data login dari penyimpanan browser
            localStorage.removeItem('activeUser');
            sessionStorage.removeItem('isLoggedIn');
            
            // Hentikan suara Google TTS jika masih berbicara
            if ('speechSynthesis' in window) window.speechSynthesis.cancel();
...
// ==================== NAVIGASI TOMBOL BAWAH (BOTTOM NAV) ====================
// 🔑 PERBAIKAN UTAMA: Mengaktifkan kembali fungsi klik untuk menu Beranda, Materi, dan Profil
document.querySelectorAll(".nav-item").forEach(el => {
    el.addEventListener("click", (e) => {
        // Cegah siswa pindah halaman jika sedang di tengah-tengah kuis
        if (quizActive) { 
        ...
        if (page === "home") { 
            ...
            currentPage = "materi"; 
            currentSubPage = null; 
            renderMateriUtama(); // Menampilkan menu IPAS, Matematika, B.Indonesia
        }
        else if (page === "profil") { 
            currentPage = "profil"; 
            renderProfil(); // Menampilkan nama siswa, nomor absen, dan tombol log out
        }
    });
});

// ==================== NAVIGASI TOMBOL BACK HEADER ====================
...
// ==================== SAKLAR TOMBOL SUARA (MUTE/UNMUTE) ====================

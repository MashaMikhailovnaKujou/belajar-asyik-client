...
// State aplikasi - Dibuat dinamis agar tidak mengunci saat proses login berlangsung
function inisialisasiUser() {
    try {
        const savedUser = localStorage.getItem('activeUser');
        return savedUser ? JSON.parse(savedUser) : null;
    } catch (e) {
        console.error("Gagal membaca session localStorage:", e);
        return null;
    }
}

window.currentUser = inisialisasiUser();
window.soundOn = true;
...
// 🔒 GERBANG KEAMANAN VALIDASI LOG IN (Diperbarui agar tidak memicu loop/stuck)
const ruteSekarang = window.location.href.toLowerCase();
const statusBukanDiLogin = !ruteSekarang.includes("login.html");
const statusBukanDiGuru = !ruteSekarang.includes("guru.html");

if (!window.currentUser && statusBukanDiLogin && statusBukanDiGuru) {
    // Jika data login kosong di HP dan user tersesat di halaman lain, kembalikan ke login
...
function render() {
    // Refresh data user aktif setiap kali fungsi render dipanggil demi kelancaran sesi kuis & profil
    window.currentUser = inisialisasiUser();
    
    // Fallback jika render dipanggil sebelum user login (mencegah error crash frontend)
    if (!window.currentUser) {
        window.currentUser = { id_user: 2, username: "Siswa", role: "siswa", avatar: "🐰" };
    }
...
function renderHome() {
    ...
    // 🛡️ SAKLAR PENYELAMAT BERANDA DARI NULL/UNDEFINED
    const userAktif = window.currentUser ? window.currentUser : { username: "Siswa", avatar: "🐰" };
    const avatarAktif = (!userAktif.avatar || userAktif.avatar === "undefined") ? "🐰" : userAktif.avatar;
    const namaTampil = userAktif.username ? userAktif.username : "Siswa";
    ...
    contentDiv.innerHTML = `
        <div class="profile-section">
            ...
            <div class="greeting">
                ...
                <p>Selamat datang kembali, <strong>${namaTampil}</strong>! ${randomMessage}</p>
            </div>
        </div>
        <div class="info-box">
...
function renderSubMateri() {
    ...
    if (typeof window.speakText === "function") {
        window.speakText(`Hore! Sekarang kita akan belajar ${mapelName} kelas 3 SD. Silakan pilih materi yang ingin kamu pelajari. Selamat belajar, ya!`);
    }
}
...
function showDetailMateri(topicId) {
    ...
    if (typeof window.speakText === "function") {
        window.speakText(`Sekarang kita akan belajar tentang ${topicData.judul}. Silakan dibaca dan didengarkan dengan saksama.`);
    }
...
function startQuizWithQuestions() {
    ...
    setTimeout(() => {
        if (typeof window.speakText === "function") {
            window.speakText(`Halo, Sobat Hebat! Selamat datang di kuis seru! Total ada ${quizQuestions.length} soal. Jawablah dengan benar untuk mendapatkan bintang sebanyak-banyaknya. Kamu pasti bisa! Ayo mulai!`);
        }
    }, 500);
}

function renderQuiz() {
    // 🛡️ BAUT PENGUNCI DARURAT: Jika array soal kuis masih kosong/belum siap, hentikan render agar tidak crash!
    if (!quizQuestions || quizQuestions.length === 0) {
        console.warn("⚠️ Mencegah Crash: Array soal kuis terdeteksi kosong.");
        quizActive = false;
        currentPage = "home";
        renderHome();
        return;
    ...
    if (currentQuizIndex >= quizQuestions.length) {
        ...
        // 🛡️ SAKLAR PENYELAMAT: Validasi data kuis sebelum dikirim ke backend Node.js
        const userKuis = window.currentUser ? window.currentUser : { id_user: 2, username: "Siswa", no_absen: 0 };
        const jumlahSoalAman = quizQuestions.length;

        fetch('https://belajar-asyik-frontend-production.up.railway.app/api/scores', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                id_user: userKuis.id_user || 2,
                nama_siswa: userKuis.username || "Siswa",
                no_absen: userKuis.no_absen || 0,
                jenis_ujian: currentMateriType ? 'latihan_topik' : 'kuis_umum',
                topik_or_bab: namaBabAtauTopik,
                skor_didapat: quizScore,
                total_soal: jumlahSoalAman
            })
        })
        .then(async res => {
        const data = await res.json();
        if (!res.ok) {
            // Jika backend membalas error (absen duplikat)
        ...
        if (percent === 100) {
            if (typeof window.speakText === "function") window.speakText(`Wah, luar biasa! Kamu mendapatkan nilai sempurna ${quizScore} dari ${quizQuestions.length}! Kamu benar-benar hebat! Pertahankan semangat belajarmu, ya!`);
        } else if (percent >= 70) {
            if (typeof window.speakText === "function") window.speakText(`Bagus sekali! Nilai kamu ${quizScore} dari ${quizQuestions.length}. Kamu pintar! Terus belajar agar lebih hebat lagi!`);
        } else if (percent >= 50) {
            if (typeof window.speakText === "function") window.speakText(`Kamu mendapatkan nilai ${quizScore} dari ${quizQuestions.length}. Cukup bagus! Ayo coba lagi supaya nilainya lebih baik!`);
        } else {
            if (typeof window.speakText === "function") window.speakText(`Kamu mendapatkan nilai ${quizScore} dari ${quizQuestions.length}. Jangan menyerah ya! Coba pelajari materi lagi, nanti kamu pasti bisa dapat nilai bagus! Semangat!`);
        }
        ...
        contentDiv.innerHTML = `
            <div class="quiz-container">
                ...
                <button id="restartQuiz" style="background:#FF9800; padding:12px; border-radius:50px; width:100%; border:none; font-weight:bold; cursor:pointer; margin-bottom:10px;">🔄 Ulangi Kuis</button>
                <button class="back-btn" id="exitQuizFinish" style="width:100%; margin:0;">🏠 Selesai & Kembali</button>
            </div>
        `;
    ...
    // Antrean pengaman ekstra: Cek jika objek soal corrupt di tengah jalan
    if (!q || !q.pilihan) {
        console.error("Mendeteksi kerusakan struktur soal kuis.");
        quizActive = false;
        render();
        return;
    }
    ...
    window.readQuizQuestion = function() {
        ...
        if (typeof window.speakText === "function") window.speakText(textToSpeak);
    ...
    q.pilihan.forEach((opt, idx) => {
        ...
        btn.onclick = () => {
            if (opt === q.jawaban) {
                quizScore++;
                if (typeof window.speakText === "function") window.speakText(`Hore! Jawaban ${abjad}. ${opt} benar! Kamu hebat! +1 bintang untukmu!`);
                createParticles();
            } else {
                const correctAbjad = String.fromCharCode(65 + q.pilihan.indexOf(q.jawaban));
                if (typeof window.speakText === "function") window.speakText(`Wah, jawaban ${abjad}. ${opt} kurang tepat. Jawaban yang benar adalah ${correctAbjad}. ${q.jawaban}. Jangan menyerah, coba lagi di soal berikutnya ya!`);
            }
            currentQuizIndex++;
            renderQuiz();
        };
    ...
    document.getElementById("exitQuizBtn")?.addEventListener("click", () => { 
        quizActive = false; 
        if (typeof window.speakText === "function") window.speakText("Kuis dihentikan. Ayo kembali lagi nanti untuk bermain kuis!");
        render(); 
    });
}
...
function renderProfil() {
    ...
    // 🛡️ SAKLAR PENYELAMAT PROFIL DARI NULL/UNDEFINED
    const userAktif = window.currentUser ? window.currentUser : { username: "Siswa", avatar: "🐰", no_absen: 0 };
    const avatarProfilAktif = (!userAktif.avatar || userAktif.avatar === "undefined") ? "🐰" : userAktif.avatar;
    const namaTampil = userAktif.username ? userAktif.username : "Siswa";
    const absenTampil = userAktif.no_absen ? userAktif.no_absen : "-";
    ...
    contentDiv.innerHTML = `
        <div class="profile-section" style="flex-direction:column; text-align:center;">
            ...
            <h2>${namaTampil}</h2>
            <p>Nomor Absen: <strong>${absenTampil}</strong></p>
            ...
            <button id="logoutBtn" style="background:#EF5350; border:none; padding:8px 20px; border-radius:40px; margin-top:15px; color:white; font-weight:bold; cursor:pointer;">🚪 Keluar Akun</button>
        </div>
    ...
    `;
    document.getElementById("gantiAvatarProfil")?.addEventListener("click", () => { changeAvatar(); renderProfil(); });
    document.getElementById("avatarProfil")?.addEventListener("click", () => { changeAvatar(); renderProfil(); });
...
document.querySelectorAll(".nav-item").forEach(el => {
    el.addEventListener("click", (e) => {
        if (quizActive) { 
            if (typeof window.speakText === "function") window.speakText("Selesaikan kuis dulu ya sebelum pindah halaman! Semangat!");
...
document.getElementById("toggleSound")?.addEventListener("click", () => {
    ...
    if (window.soundOn) {
        if (typeof window.speakText === "function") window.speakText("Suara diaktifkan! Selamat belajar dan bersenang-senang!");
    } else {
        if ('speechSynthesis' in window) window.speechSynthesis.cancel();
    }
});

// Jalankan inisialisasi render perdana secara aman
render();